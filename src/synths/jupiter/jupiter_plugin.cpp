/*
 * jupiter_plugin.cpp - Move Anything plugin wrapper for Jupiter-8
 */
...
struct JupiterInstance {
    jupiter_engine_t engine;
...
static void apply_preset(JupiterInstance *inst, int idx) {
    if (idx < 0 || idx >= inst->preset_count) return;
    const bristol_patch_t *p = &inst->presets[idx];
    jupiter_engine_t *e = &inst->engine;
    jupiter_engine_reset(e);

    /* Jupiter-8 parameter mapping - approximate */
    ...
    e->osc1_tri = get_p(p, 7) > 0.5f ? 1 : 0;
    e->osc1_pw = get_p(p, 8, 0.5f);
    e->osc2_octave = (int)(get_p(p, 9) * 5.99f);
    e->osc2_detune = get_p(p, 10);
    e->osc2_saw = get_p(p, 11) > 0.5f ? 1 : 0;
    e->osc2_pulse = get_p(p, 12) > 0.5f ? 1 : 0;
    e->osc2_noise = get_p(p, 13) > 0.5f ? 1 : 0;
    e->osc2_pw = get_p(p, 14, 0.5f);
    e->osc_sync = get_p(p, 15) > 0.5f ? 1 : 0;
    e->osc_xmod = get_p(p, 16) > 0.5f ? 1 : 0;
    e->mix_osc1 = get_p(p, 17, 0.5f);
    e->mix_osc2 = get_p(p, 18, 0.5f);
    e->filter_cutoff = get_p(p, 19, 0.7f);
    e->filter_resonance = get_p(p, 20);
    e->filter_env_amt = get_p(p, 21, 0.5f);
    e->filter_kbd_track = get_p(p, 22) > 0.5f ? 1 : 0;
...
static int load_presets(JupiterInstance *inst) {
    char path[512];
    snprintf(path, sizeof(path), "%s/presets/jupiter", inst->module_dir);
    ...
    if (count <= 0) {
        ...
        inst->presets[0].params[5] = 1.0f; /* osc1 saw */
        inst->presets[0].params[11] = 1.0f; /* osc2 saw */
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg), "[bristol-jupiter] Loaded %d presets", count);
        plugin_log(msg);
    }
    return count;
}

static void* jupiter_create(const char *module_dir, const char *json_defaults) {
    (void)json_defaults;
    JupiterInstance *inst = (JupiterInstance*)calloc(1, sizeof(JupiterInstance));
    if (!inst) return NULL;
    strncpy(inst->module_dir, module_dir ? module_dir : ".", sizeof(inst->module_dir) - 1);
    jupiter_engine_init(&inst->engine);
    inst->gain = 1.0f;
    inst->preset_count = load_presets(inst);
    if (inst->preset_count > 0) apply_preset(inst, 0);
    plugin_log("[bristol-jupiter] Instance created");
    return inst;
}

static void jupiter_destroy(void *instance) { if (instance) free(instance); }

static void jupiter_on_midi(void *instance, const uint8_t *msg, int len, int source) {
    JupiterInstance *inst = (JupiterInstance *)instance;
    if (len < 1) return;
    uint8_t status = msg[0] & 0xF0;
    switch (status) {
        case 0x90:
            if (len >= 3 && msg[2] > 0)
                jupiter_engine_note_on(&inst->engine, msg[1] + inst->octave_transpose * 12, msg[2] / 127.0f);
            else if (len >= 2)
                jupiter_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0x80:
            if (len >= 2) jupiter_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0xE0:
            if (len >= 3) jupiter_engine_pitch_bend(&inst->engine, (((msg[2] << 7) | msg[1]) - 8192) / 8192.0f);
            break;
        case 0xB0:
            if (len >= 3) {
                ...
                switch (msg[1]) {
                    ...
                    case 123: jupiter_engine_all_notes_off(&inst->engine); break;
                }
            }
            break;
    }
...
static void jupiter_set_param(void *instance, const char *key, const char *val) {
    JupiterInstance *inst = (JupiterInstance *)instance;
...
static int jupiter_get_param(void *instance, const char *key, char *buf, int buf_len) {
    JupiterInstance *inst = (JupiterInstance *)instance;
...
static int jupiter_get_error(void *inst, char *buf, int len) { (void)inst;(void)buf;(void)len; return 0; }

static void jupiter_render(void *instance, int16_t *out_lr, int frames) {
    JupiterInstance *inst = (JupiterInstance *)instance;
    if (!inst) { memset(out_lr, 0, frames * 4); return; }
    float mono_buf[256];
    if (frames > 256) frames = 256;
    jupiter_engine_render(&inst->engine, mono_buf, frames);
...
static plugin_api_v2_t g_api = {
    .api_version = 2, .create_instance = jupiter_create, .destroy_instance = jupiter_destroy,
    .on_midi = jupiter_on_midi, .set_param = jupiter_set_param, .get_param = jupiter_get_param,
    .get_error = jupiter_get_error, .render_block = jupiter_render
};

extern "C" plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    g_host = host;
    plugin_log("[bristol-jupiter] Plugin v2 initialized");
    return &g_api;
}
