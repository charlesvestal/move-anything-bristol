/*
 * jupiter_engine.c - Roland Jupiter-8 style synthesizer engine
 */

#include "jupiter_engine.h"
#include <string.h>

static int find_voice(jupiter_engine_t *e) {
    for (int i = 0; i < JUPITER_MAX_VOICES; i++)
        if (!e->voices[i].active) return i;
    return 0;
}

static int find_voice_for_note(jupiter_engine_t *e, int note) {
    for (int i = 0; i < JUPITER_MAX_VOICES; i++)
        if (e->voices[i].active && e->voices[i].note == note) return i;
    return -1;
}

void jupiter_engine_init(jupiter_engine_t *e) {
    memset(e, 0, sizeof(jupiter_engine_t));
    e->sample_rate = BRISTOL_SAMPLE_RATE;

    for (int i = 0; i < JUPITER_MAX_VOICES; i++) {
        jupiter_voice_t *v = &e->voices[i];
...
void jupiter_engine_reset(jupiter_engine_t *e) {
    for (int i = 0; i < JUPITER_MAX_VOICES; i++) {
        jupiter_voice_t *v = &e->voices[i];
        v->active = 0;
        env_init(&v->filter_env);
...
void jupiter_engine_note_on(jupiter_engine_t *e, int note, float velocity) {
    int vi = find_voice(e);
    jupiter_voice_t *v = &e->voices[vi];
...
void jupiter_engine_note_off(jupiter_engine_t *e, int note) {
...
void jupiter_engine_pitch_bend(jupiter_engine_t *e, float bend) { e->pitch_bend = bend; }

void jupiter_engine_all_notes_off(jupiter_engine_t *e) {
    for (int i = 0; i < JUPITER_MAX_VOICES; i++) {
        e->voices[i].active = 0;
...
void jupiter_engine_render(jupiter_engine_t *e, float *output, int frames) {
    ...
    for (int vi = 0; vi < JUPITER_MAX_VOICES; vi++) {
        jupiter_voice_t *v = &e->voices[vi];
        ...
        for (int i = 0; i < frames; i++) {
            ...
            if (e->osc1_tri) { v->osc1.wave = WAVE_TRI; osc1_out += osc_generate(&v->osc1, freq1, sr) * 0.8f; }
            if (e->osc1_pulse) {
                float pw = e->osc1_pw + lfo_val * e->lfo_to_pw * 0.4f;
                pw = clampf(pw, 0.1f, 0.9f);
                osc1_out += ((float)v->osc1.phase < pw ? 1.0f : -1.0f) * 0.8f;
            }
            if (!e->osc1_saw && !e->osc1_tri && !e->osc1_pulse) {
            ...
            if (e->osc2_noise) osc2_out += noise_white(&e->noise) * 0.5f;
            if (!e->osc2_saw && !e->osc2_pulse && !e->osc2_noise) {
