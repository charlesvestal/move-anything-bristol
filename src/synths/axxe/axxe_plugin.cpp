/*
 * axxe_plugin.cpp - Move Anything plugin wrapper for ARP Axxe
 */
...
static void apply_preset(AxxeInstance *inst, int idx) {
    if (idx < 0 || idx >= inst->preset_count) return;
    const bristol_patch_t *p = &inst->presets[idx];
    axxe_engine_t *e = &inst->engine;
    axxe_engine_reset(e);
    ...
    e->osc_square = get_p(p, 7) > 0.5f ? 1 : 0;
    e->osc_pw = get_p(p, 8, 0.5f);
    e->noise_level = get_p(p, 9);
    ...
    e->master_volume = get_p(p, 22, 0.8f);

    inst->current_preset = idx;
}

static int load_presets(AxxeInstance *inst) {
    char path[512];
    snprintf(path, sizeof(path), "%s/presets/axxe", inst->module_dir);
    ...
    if (count <= 0) {
        ...
        char msg[256];
        snprintf(msg, sizeof(msg), "[bristol-axxe] Loaded %d presets", count);
        plugin_log(msg);
    }
    return count;
}

static void* axxe_create(const char *module_dir, const char *json_defaults) {
    (void)json_defaults;
    AxxeInstance *inst = (AxxeInstance*)calloc(1, sizeof(AxxeInstance));
    if (!inst) return NULL;
    strncpy(inst->module_dir, module_dir ? module_dir : ".", sizeof(inst->module_dir) - 1);
    axxe_engine_init(&inst->engine);
    inst->gain = 1.0f;
    inst->preset_count = load_presets(inst);
    if (inst->preset_count > 0) apply_preset(inst, 0);
    plugin_log("[bristol-axxe] Instance created");
    return inst;
}

static void axxe_destroy(void *instance) { if (instance) free(instance); }

static void axxe_on_midi(void *instance, const uint8_t *msg, int len, int source) {
    AxxeInstance *inst = (AxxeInstance *)instance;
    if (len < 1) return;
    uint8_t status = msg[0] & 0xF0;
    switch (status) {
        case 0x90:
            if (len >= 3 && msg[2] > 0)
                axxe_engine_note_on(&inst->engine, msg[1] + inst->octave_transpose * 12, msg[2] / 127.0f);
            else if (len >= 2)
                axxe_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0x80:
            if (len >= 2) axxe_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0xE0:
            if (len >= 3) axxe_engine_pitch_bend(&inst->engine, (((msg[2] << 7) | msg[1]) - 8192) / 8192.0f);
            break;
        case 0xB0:
            if (len >= 3) {
                ...
                switch (msg[1]) {
                    ...
                    case 123: axxe_engine_all_notes_off(&inst->engine); break;
                }
            }
            break;
    }
...
static void axxe_set_param(void *instance, const char *key, const char *val) {
    AxxeInstance *inst = (AxxeInstance *)instance;
...
static int axxe_get_param(void *instance, const char *key, char *buf, int buf_len) {
    AxxeInstance *inst = (AxxeInstance *)instance;
...
static int axxe_get_error(void *inst, char *buf, int len) { (void)inst;(void)buf;(void)len; return 0; }

static void axxe_render(void *instance, int16_t *out_lr, int frames) {
    AxxeInstance *inst = (AxxeInstance *)instance;
    if (!inst) { memset(out_lr, 0, frames * 4); return; }
    float mono_buf[256];
    if (frames > 256) frames = 256;
    axxe_engine_render(&inst->engine, mono_buf, frames);
...
static plugin_api_v2_t g_api = {
    .api_version = 2, .create_instance = axxe_create, .destroy_instance = axxe_destroy,
    .on_midi = axxe_on_midi, .set_param = axxe_set_param, .get_param = axxe_get_param,
    .get_error = axxe_get_error, .render_block = axxe_render
};

extern "C" plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    g_host = host;
    plugin_log("[bristol-axxe] Plugin v2 initialized");
    return &g_api;
}
