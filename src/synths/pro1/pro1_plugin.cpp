/*
 * pro1_plugin.cpp - Move Anything plugin wrapper for Pro-One
 */
...
static void apply_preset(Pro1Instance *inst, int idx) {
    if (idx < 0 || idx >= inst->preset_count) return;
    const bristol_patch_t *p = &inst->presets[idx];
    pro1_engine_t *e = &inst->engine;
    pro1_engine_reset(e);

    /* Pro-One parameter mapping */
    e->lfo.rate = get_p(p, 0);
    e->lfo.wave = (bristol_wave_t)(int)(get_p(p, 1) * 3.99f);
    e->lfo_to_osc = get_p(p, 2);
    e->lfo_to_filter = get_p(p, 3);
    e->lfo_to_pw = get_p(p, 4);
    e->osc_a_octave = (int)(get_p(p, 5) * 5.99f);
    e->osc_a_saw = get_p(p, 6) > 0.5f ? 1 : 0;
    e->osc_a_pulse = get_p(p, 7) > 0.5f ? 1 : 0;
    e->osc_a_pw = get_p(p, 8, 0.5f);
    e->osc_b_octave = (int)(get_p(p, 9) * 5.99f);
    e->osc_b_fine = get_p(p, 10);
    e->osc_b_saw = get_p(p, 11) > 0.5f ? 1 : 0;
    e->osc_b_tri = get_p(p, 12) > 0.5f ? 1 : 0;
    e->osc_b_pulse = get_p(p, 13) > 0.5f ? 1 : 0;
    e->osc_b_pw = get_p(p, 14, 0.5f);
    e->osc_sync = get_p(p, 15) > 0.5f ? 1 : 0;
    e->mix_osc_a = get_p(p, 16, 0.5f);
    e->mix_osc_b = get_p(p, 17, 0.5f);
...
static int load_presets(Pro1Instance *inst) {
    char path[512];
    snprintf(path, sizeof(path), "%s/presets/pro1", inst->module_dir);
    ...
    if (count <= 0) {
        ...
        inst->presets[0].params[11] = 1.0f;
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg), "[bristol-pro1] Loaded %d presets", count);
        plugin_log(msg);
    }
    return count;
}

static void* pro1_create(const char *module_dir, const char *json_defaults) {
    (void)json_defaults;
    Pro1Instance *inst = (Pro1Instance*)calloc(1, sizeof(Pro1Instance));
    if (!inst) return NULL;
    strncpy(inst->module_dir, module_dir ? module_dir : ".", sizeof(inst->module_dir) - 1);
    pro1_engine_init(&inst->engine);
    inst->gain = 1.0f;
    inst->preset_count = load_presets(inst);
    if (inst->preset_count > 0) apply_preset(inst, 0);
    plugin_log("[bristol-pro1] Instance created");
    return inst;
}

static void pro1_destroy(void *instance) { if (instance) free(instance); }

static void pro1_on_midi(void *instance, const uint8_t *msg, int len, int source) {
    Pro1Instance *inst = (Pro1Instance *)instance;
    if (len < 1) return;
    uint8_t status = msg[0] & 0xF0;
    switch (status) {
        case 0x90:
            if (len >= 3 && msg[2] > 0)
                pro1_engine_note_on(&inst->engine, msg[1] + inst->octave_transpose * 12, msg[2] / 127.0f);
            else if (len >= 2)
                pro1_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0x80:
            if (len >= 2) pro1_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0xE0:
            if (len >= 3) pro1_engine_pitch_bend(&inst->engine, (((msg[2] << 7) | msg[1]) - 8192) / 8192.0f);
            break;
        case 0xB0:
            if (len >= 3) {
                float val = msg[2] / 127.0f;
                switch (msg[1]) {
                    case 1: pro1_engine_mod_wheel(&inst->engine, val); break;
                    case 74: inst->engine.filter_cutoff = val; break;
                    case 71: inst->engine.filter_resonance = val; break;
                    case 123: pro1_engine_all_notes_off(&inst->engine); break;
                }
            }
            break;
    }
...
static void pro1_set_param(void *instance, const char *key, const char *val) {
    Pro1Instance *inst = (Pro1Instance *)instance;
...
static int pro1_get_param(void *instance, const char *key, char *buf, int buf_len) {
    Pro1Instance *inst = (Pro1Instance *)instance;
...
static int pro1_get_error(void *inst, char *buf, int len) { (void)inst;(void)buf;(void)len; return 0; }

static void pro1_render(void *instance, int16_t *out_lr, int frames) {
    Pro1Instance *inst = (Pro1Instance *)instance;
    if (!inst) { memset(out_lr, 0, frames * 4); return; }
    float mono_buf[256];
    if (frames > 256) frames = 256;
    pro1_engine_render(&inst->engine, mono_buf, frames);
...
static plugin_api_v2_t g_api = {
    .api_version = 2, .create_instance = pro1_create, .destroy_instance = pro1_destroy,
    .on_midi = pro1_on_midi, .set_param = pro1_set_param, .get_param = pro1_get_param,
    .get_error = pro1_get_error, .render_block = pro1_render
};

extern "C" plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    g_host = host;
    plugin_log("[bristol-pro1] Plugin v2 initialized");
    return &g_api;
}
