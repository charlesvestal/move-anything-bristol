/*
 * bit1_engine.c - Crumar Bit-1 style synthesizer engine
 */

#include "bit1_engine.h"
#include <string.h>

static int find_voice(bit1_engine_t *e) {
    for (int i = 0; i < BIT1_MAX_VOICES; i++)
        if (!e->voices[i].active) return i;
    return 0;
}

static int find_voice_for_note(bit1_engine_t *e, int note) {
    for (int i = 0; i < BIT1_MAX_VOICES; i++)
...
void bit1_engine_init(bit1_engine_t *e) {
    memset(e, 0, sizeof(bit1_engine_t));
    e->sample_rate = BRISTOL_SAMPLE_RATE;

    for (int i = 0; i < BIT1_MAX_VOICES; i++) {
        bit1_voice_t *v = &e->voices[i];
...
void bit1_engine_pitch_bend(bit1_engine_t *e, float bend) { e->pitch_bend = bend; }
...
static float gen_dco_wave(bristol_osc_t *osc, int wave, float pw, float freq, float sr) {
    float out = 0.0f;
    switch (wave) {
        case 0: /* Saw */
            osc->wave = WAVE_SAW;
            out = osc_generate(osc, freq, sr);
            break;
        case 1: /* Square */
            out = ((float)osc->phase < 0.5f ? 1.0f : -1.0f) * 0.8f;
            osc->phase += freq / sr; if (osc->phase >= 1.0) osc->phase -= 1.0;
            break;
        case 2: /* Pulse */
            out = ((float)osc->phase < pw ? 1.0f : -1.0f) * 0.8f;
            osc->phase += freq / sr; if (osc->phase >= 1.0) osc->phase -= 1.0;
            break;
        default:
            osc->wave = WAVE_SAW;
...
void bit1_engine_render(bit1_engine_t *e, float *output, int frames) {
    ...
    for (int vi = 0; vi < BIT1_MAX_VOICES; vi++) {
        bit1_voice_t *v = &e->voices[vi];
        ...
        for (int i = 0; i < frames; i++) {
            ...
            float osc1_out = gen_dco_wave(&v->osc1, e->osc1_wave, clampf(pw1, 0.1f, 0.9f), freq1, sr);
            float osc2_out = gen_dco_wave(&v->osc2, e->osc2_wave, clampf(pw2, 0.1f, 0.9f), freq2, sr);
