/*
 * bit1_plugin.cpp - Move Anything plugin wrapper for Bit-1
 */
...
static void apply_preset(Bit1Instance *inst, int idx) {
    if (idx < 0 || idx >= inst->preset_count) return;
    const bristol_patch_t *p = &inst->presets[idx];
    bit1_engine_t *e = &inst->engine;
    bit1_engine_reset(e);
    ...
    e->mix_osc1 = get_p(p, 11, 0.5f);
    e->mix_osc2 = get_p(p, 12, 0.5f);
    e->mix_noise = get_p(p, 13);
    e->filter_cutoff = get_p(p, 14, 0.7f);
    e->filter_resonance = get_p(p, 15);
    e->filter_env_amt = get_p(p, 16, 0.5f);
    e->filter_kbd_track = get_p(p, 17);
    e->fenv_a = get_p(p, 18, 0.01f);
    e->fenv_d = get_p(p, 19, 0.3f);
    e->fenv_s = get_p(p, 20, 0.5f);
    e->fenv_r = get_p(p, 21, 0.3f);
    e->aenv_a = get_p(p, 22, 0.01f);
    e->aenv_d = get_p(p, 23, 0.3f);
    e->aenv_s = get_p(p, 24, 0.7f);
    e->aenv_r = get_p(p, 25, 0.3f);
    e->master_volume = get_p(p, 26, 0.8f);

    inst->current_preset = idx;
}

static int load_presets(Bit1Instance *inst) {
    char path[512];
    snprintf(path, sizeof(path), "%s/presets/bit1", inst->module_dir);
    int count = bristol_scan_presets(path, inst->presets, MAX_PRESETS);
    if (count <= 0) {
        count = 1;
        strcpy(inst->presets[0].name, "Init");
        inst->presets[0].param_count = 27;
        for (int i = 0; i < 27; i++) inst->presets[0].params[i] = 0.5f;
    } else {
        char msg[256];
        snprintf(msg, sizeof(msg), "[bristol-bit1] Loaded %d presets", count);
        plugin_log(msg);
    }
    return count;
}

static void* bit1_create(const char *module_dir, const char *json_defaults) {
    (void)json_defaults;
    Bit1Instance *inst = (Bit1Instance*)calloc(1, sizeof(Bit1Instance));
    if (!inst) return NULL;
    strncpy(inst->module_dir, module_dir ? module_dir : ".", sizeof(inst->module_dir) - 1);
    bit1_engine_init(&inst->engine);
    inst->gain = 1.0f;
    inst->preset_count = load_presets(inst);
    if (inst->preset_count > 0) apply_preset(inst, 0);
    plugin_log("[bristol-bit1] Instance created");
    return inst;
}

static void bit1_destroy(void *instance) { if (instance) free(instance); }

static void bit1_on_midi(void *instance, const uint8_t *msg, int len, int source) {
    Bit1Instance *inst = (Bit1Instance *)instance;
    if (len < 1) return;
    uint8_t status = msg[0] & 0xF0;
    switch (status) {
        case 0x90:
            if (len >= 3 && msg[2] > 0)
                bit1_engine_note_on(&inst->engine, msg[1] + inst->octave_transpose * 12, msg[2] / 127.0f);
            else if (len >= 2)
                bit1_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0x80:
            if (len >= 2) bit1_engine_note_off(&inst->engine, msg[1] + inst->octave_transpose * 12);
            break;
        case 0xE0:
            if (len >= 3) bit1_engine_pitch_bend(&inst->engine, (((msg[2] << 7) | msg[1]) - 8192) / 8192.0f);
            break;
        case 0xB0:
            if (len >= 3) {
                ...
                switch (msg[1]) {
                    ...
                    case 123: bit1_engine_all_notes_off(&inst->engine); break;
                }
            }
            break;
    }
...
static void bit1_set_param(void *instance, const char *key, const char *val) {
    Bit1Instance *inst = (Bit1Instance *)instance;
...
static int bit1_get_param(void *instance, const char *key, char *buf, int buf_len) {
    Bit1Instance *inst = (Bit1Instance *)instance;
...
static int bit1_get_error(void *inst, char *buf, int len) { (void)inst;(void)buf;(void)len; return 0; }

static void bit1_render(void *instance, int16_t *out_lr, int frames) {
    Bit1Instance *inst = (Bit1Instance *)instance;
    if (!inst) { memset(out_lr, 0, frames * 4); return; }
    float mono_buf[256];
    if (frames > 256) frames = 256;
    bit1_engine_render(&inst->engine, mono_buf, frames);
...
static plugin_api_v2_t g_api = {
    .api_version = 2, .create_instance = bit1_create, .destroy_instance = bit1_destroy,
    .on_midi = bit1_on_midi, .set_param = bit1_set_param, .get_param = bit1_get_param,
    .get_error = bit1_get_error, .render_block = bit1_render
};

extern "C" plugin_api_v2_t* move_plugin_init_v2(const host_api_v1_t *host) {
    g_host = host;
    plugin_log("[bristol-bit1] Plugin v2 initialized");
    return &g_api;
}
